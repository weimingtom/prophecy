...
function PBPack1(user, pass)
	...
	bytes2 = bytes2..writepbstring(pbtype, bytes3)
	bytes = bytes..Bit8toStr(MODL_SERIALIZER)
	bytes = bytes..Bit8toStr(PROTOBUF)
	bytes = bytes..bytes2
...
xpcall(function ()
	for k = 1, 100000 do
		...
		c:send(PBPack1("nanami", "12345")) -- 23 bytes
		c:send(PBPack2("nanami", k, 3)) -- 24 bytes
		c:send(PBPack1("nanami", "12345")) -- 23 bytes
		if true then
